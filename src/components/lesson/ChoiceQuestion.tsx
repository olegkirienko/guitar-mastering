...
interface ChoiceQuestionProps { question: string; choices: readonly ChoiceQuestionChoice[]; correctChoiceId: string; checkLabel?: string; onCheck?: (choiceId: string, isCorrect: boolean) => void; }

export function ChoiceQuestion({ question, choices, correctChoiceId, checkLabel = 'Перевірити', onCheck }: ChoiceQuestionProps) {
  ...
  return <fieldset className="space-y-4" aria-describedby={checked ? `${id}-feedback` : undefined}>
    ...
    {checked && selected && <div id={`${id}-feedback`} role="status" className="flex gap-3 rounded-lg bg-gray-50 p-4 text-sm leading-6 text-gray-700">{isCorrect ? <CheckCircle className="mt-0.5 size-5 shrink-0 text-success-600" /> : <HelpCircle className="mt-0.5 size-5 shrink-0 text-brand-700" />}<p><strong>{isCorrect ? 'Так.' : 'Це припущення можна перевірити.'}</strong> {selected.feedback}</p></div>}
  </fieldset>;
}
